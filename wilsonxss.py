...
def populate_list():
	payload_lit= []
	r = open("xss_payload_list.txt",'r')
	for line in r.readlines():
		payload_lit.append(str(line))
	r.close()
	return payload_lit	
...
def start():
	...
	if getopt.u:
		...
		crawler.crawl(getopt.u,int(getopt.depth),getopt.proxy,getopt.user_agent,check(getopt),getopt.method,populate_list())
