...
class core:
		...
		for form in forms:
			...
			if form["method"].lower().strip() == "post":
				...
				for key in form.find_all(["input","textarea"]):
					for xss_payload in self.payload_list:
						try:
							if key["type"] == "submit":
								...
								Log.info("Form key name: "+G+key["name"]+N+" value: "+G+xss_payload)
								keys.update({key["name"]:xss_payload})
			...
			if url.startswith("http://") is False or url.startswith("https://") is False or url.startswith("mailto:") is False:
				...
				if query != "":
					for xss_payload in self.payload_list:
						...
						query_payload=query.replace(query[query.find("=")+1:len(query)],xss_payload,1)
						test=base.replace(query,query_payload,1)
						
						query_all=base.replace(query,urlencode({x: xss_payload for x in parse_qs(query)}))
	...
	@classmethod
	def main(self,url,proxy,headers,payload,payload_list,method=2,):
